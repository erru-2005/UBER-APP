...
function ConfirmRidePopUp(props) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      if (otp) {
        const resp = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/rides/start-ride`, {
          params: {
        ...
        if (resp.status === 200) {
          navigate('/riding-detail', {
            state: {
              rideData: resp.data, //  data
            },
          });
        }
      }
      } catch (err) {
        console.error("Error in submitHandler:", err.response?.data || err.message);
      }
    };
    return (
      <div className="w-full  bg-white overflow-auto">
        <div className=" p-5 border-gray-300 rounded-lg  flex flex-col justify-center items-center w-full h-[90%]">
          ...
          <div className="flex items-center justify-between  w-full  px-4 py-2 rounded-lg border-2 border-yellow-500">
            <div className="flex items-center justify-start gap-4 w-[80%]">
              ...
              <h2 className="text-2xl font-bold capitalize">{props.newRide?.userId.fullname.firstname}</h2>
            </div>
            <div className="flex flex-col items-start mt-4 justify-center">
              <h3 className="text-xl font-bold">{props.newRide?.distance}</h3>
            </div>
          </div>
          <div className="flex flex-col justify-center items-start  pt-4 w-full">
            ...
            <div className="flex gap-4 items-center p-2 border-b-2 border-gray-300 w-full ">
              ...
              <div className="flex  flex-col">
                ...
                <p className="text-sm">{props.newRide?.pickup}</p>
              </div>
            </div>
            {/*to address8*/}
            <div className="flex gap-4 items-center mt-2.5 p-2 border-b-2 border-gray-300 w-full">
              ...
              <div className="flex flex-col ">
                ...
                <p className="text-sm">{props.newRide?.dropoff}</p>
              </div>
            </div>
            {/* price */}
            <div className="flex gap-4 items-center  mt-2.5 p-2">
              ...
              <div className="flex flex-col ">
                <h3 className="font-bold">₹ {props.newRide?.fare}</h3>
          ...
          <form className="w-full flex flex-col items-center">
            <input
              ...
              type="number"
              className="p-4 border-2 py-2 mt-4 mb-2 w-[90%] rounded-2xl bg-gray-200"
            ...
            <button
              ...
              className=" bg-green-600 text-white text-center font-semibold rounded-lg px-4 py-2 mt-4 mb-2 w-[90%] d-block"
            >
              Confirm Ride
            </button>
            <button
              ...
              className=" bg-red-800 text-white rounded-lg px-4 py-2 mt-4 mb-2 w-[90%]"
            >
              cancel
            </button>
          </form>
